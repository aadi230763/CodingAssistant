...
export class GitService {
  ...
  public async cloneUserRepo(url: string): Promise<RepositoryInfo> {
    ...
    try {
      ...
      const localPath = `${RNFS.DocumentDirectoryPath}/repositories/${repoId}`;
      ...
      await RNFS.mkdir(`${RNFS.DocumentDirectoryPath}/repositories`).catch(() => {
  ...
  private async triggerSecurityAnalysis(repository: RepositoryInfo): Promise<void> {
    ...
    try {
      ...
      const packageJsonExists = await RNFS.exists(packageJsonPath);
      ...
      const packageContent = await RNFS.readFile(packageJsonPath, 'utf8');
  ...
  public async deleteRepository(repositoryId: string): Promise<void> {
    const repoPath = `${RNFS.DocumentDirectoryPath}/repositories/${repositoryId}`;
    await RNFS.unlink(repoPath);
